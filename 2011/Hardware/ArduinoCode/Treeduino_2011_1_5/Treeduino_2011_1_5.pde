 ...
 *
 */
char verNum[] = "Treeduino 2011 - v1.6";  // Saved to EEPROM in Setup method as well as printed invisibly (same color as background) in the web form.
...
void outputPins(WebServer &server, WebServer::ConnectionType type, bool addControls = false)
{
  ...
  char* strLightStrings[] = {"", "", "Tree White", "Tree Rainbow", "Banister White", "Banister Rainbow", "Ch. 4", "Red", "Green", "Blue", "", "", "", "", "RGB Fade"};
  ...
  for (i = 2; i <= 14; ++i)
  {
    ...
    if ( i == 6 ) { // Skip over Channel 4, it has not been added yet.
     i = 7; 
    }
    
    if ( i == 7 ) {
      server << "<h3>Star Control</h3>";
      server << "<br>"; 
    }
