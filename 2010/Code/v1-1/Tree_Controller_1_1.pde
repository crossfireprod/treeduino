...
void outputPins(WebServer &server, WebServer::ConnectionType type, bool addControls = false)
{
  P(htmlHead) =
    "<html>"
    "<head>"
    "<title>Cross Family Christmas Tree Controller</title>"
    "<style type=\"text/css\">"
...
/* Analog Pin Read
  ...
  server << "</p>";
  END Analog Pin Read */
  server << "<p>To apply changes, click the *Submit* button below.</p>";
  server << "<br/>";
  
  if (addControls)
    server << "<input type='submit' value='Apply Changes'/></form>";
